        ...
        <div class="box box-success">
            <?php if ($this->session->userdata('level_user') == 'admin') : ?>
                <div class="box-header text-right">
                    <a href="<?= site_url('ijin_penahanan/create') ?>" class="btn btn-success mb-3">
                        Tambah Data
                    </a>
                </div>
            <?php endif; ?>
                    ...
                    <tbody>
                        <?php foreach ($data as $row) : ?>
                            <tr>
                                ...
                                <td class="text-center">
                                    <?= ($row->status != 'SELESAI') ? diffdate(date('Y-m-d'), $row->waktu_tunggu) : 'SELESAI'  ?>
                                </td>
                                <td class=" text-center">
                                    <?php
                                    ...
                                    if (in_array($level_user, $level_array)) : ?>
                                        ...
                                        <?php // button_print('ijin_penahanan/print/' . $row->id) 
                                        ?>
                                        <?= button_show('ijin_penahanan/show/' . $row->id) ?>
                                    <?php endif; ?>
                                    ...
                                    <?php if ($level_user == 'admin') : ?>
                                        <?= button_edit('ijin_penahanan/edit/' . $row->id) ?>
                                        <?= button_delete('ijin_penahanan/delete/' . $row->id) ?>
                                    <?php endif; ?>
                                </td>
                            </tr>
                        <?php endforeach; ?>
